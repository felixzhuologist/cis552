...
> zipTree1 (Branch l r) (Branch l' r') = case ((zipTree1 l l'), (zipTree1 r r')) of
...
Pattern for returning: stick into Maybe context

-- > return :: a -> Maybe a
-- > return = Just

Pattern for using:
we have a maybe value, which may have come from a previous computation.
if it's Nothing, then we're done as we return Nothing. If it contains a value,
we apply a function to it that returns a new maybe (so that we can chain uses)
...
>   (zipTree2 l l') >>= (\t1 -> (zipTree2 r r') >>= \t2 -> return $ Branch t1 t2)
...
> monad_fmap f x = x >>= (return . f)

> monad_pure :: (Monad m) => a -> m a 
> monad_pure = return

> monad_zap  :: (Monad m) => m (a -> b) -> m a -> m b
> monad_zap mf mx = mf >>= (\f -> (mx >>= \x -> return $ f x))
...
(recall that [] counts as a failure, so you can think of it as passing a Nothing
which means that any further "do lines" or >>=s will return failures). since
sequence doesn't care about what's inside the context (in this case, what's inside the list),
we could put anything inside the list to signify success. The expansion of the 
above do notation is:
p xs ys = (xs >>= (\x -> ys >>= (\y -> if x < y then [()] else [] >> (return (x, y)))))
p xs ys = (xs >>= (\x -> ys >>= (\y -> if x < y then [()] else [] >>= (\_ -> return (x, y)))))
...
~~~~~{.haskell}
ghci> genpairs (Nothing) (Just 'a')
ghci> genpairs (Just 42) (Nothing)
ghci> genpairs (Just 2) (Just 'a')
~~~~~
...
> --  where
> --   f mresults mx = mx >>= (\x -> mresults >>= (\results -> return $ x:results))
> --   f mresults mx = (liftM2 (:)) mx mresults
