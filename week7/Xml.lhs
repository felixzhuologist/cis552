...
> import Parsers (Parser, satisfy, char, string, doParse, filter, get, tripleP)
...
> text :: Parser String
> text = some $ Parsers.filter (not . reserved) get
...
> openTag = char '<' *> text <* char '>'

> closeTag :: Parser String
> closeTag = string "</" *> text <* char '>'

> container :: Parser SimpleXML -> Parser SimpleXML
> container p = Element <$> (char '<' *> text <* char '>')
>                       <*> many p
>                       <*  (string "</" *> text <* char '>')
...
> xml = emptyContainer <|> pcdata <|> (container xml)
...
> container2 p = triplesToXML <$> (Parsers.filter matchingTags stringToTriples) where
>   stringToTriples :: Parser (String, [SimpleXML], String)
>   stringToTriples = tripleP (char '<' *> text <* char '>') (many p) (string "</" *> text <* char '>')
>   matchingTags (open, _, close) = open == close
>   triplesToXML (open, content, _) = Element open content -- open == close at this point
