...
> eval (Val v) _ = v
> eval (Var v) s = case Map.lookup v s of
>                       Just x -> x
>                       Nothing -> IntVal 0
...
>   arbitrary = liftM (V . (:[])) (elements [ 'A' .. 'Z' ])
...
>                          (2, liftM3 Op arbitrary arbitrary arbitrary)]
...
Finally, we need to write a generator for `Store` so that we can
evaluate the expression with some arbitrary store.  We randomly
generate a list of key-value tuples, which is turned into a `Map` by
...
>   shrink (Var v) = []
>   shrink (Val v) = []
>   shrink (Op op e1 e2) = [e1, e2] ++ shrink e1 ++ shrink e2
> -}
