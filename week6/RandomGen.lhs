...
> nextBounded bound s = let (result, gen) = next s in 
>   if result < bound then (result, gen) else nextBounded bound gen
...
>    arb1 gen = let (result, newGen) = next gen in (even result, newGen)
...
>    arb1 gen = ((x, y), newGen) where
>      (x, newGen') = arb1 gen
>      (y, newGen) = arb1 newGen'

And for lists?

> instance (Arb1 a) => Arb1 [a] where
>    arb1 s = let (x, s') = arb1 s in ([x], s')
