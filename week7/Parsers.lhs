...
>     fmap f p = P $ \s -> [ (f c, cs) | (c, cs) <- doParse p s ] 
...
> pairP0 p1 p2 = P $ \s -> do (x, xs) <- doParse p1 s
>                             (y, ys) <- doParse p2 xs
>                             return ((x, y), ys)
...
> string' :: String -> Parser String
> string' s = foldr f (pure "") s where
>   f :: Char -> Parser String -> Parser String
>   f c p = liftA2 (:) (char c) p
