...
> --           the   Store -> Store here is our first state transformer
...
> aux (Leaf x) = \s -> ( Leaf (x,s) ,s+1) -- maybe use returnST here somehow???
> aux (Branch t1 t2) = aux t1 `bindST` (\ t1' -> 
>                      aux t2 `bindST` (\ t2' -> 
...
With better naming:
-- bindSt stateTransformer f = (\initalState -> let (result, newState) = stateTransformer initialState in f result newState)
so bind composes state transformers (but doesn't actually perform the tranformation)
...
> mlabel :: Tree a -> ST2 (Tree (a,Int)) -- Store -> (Tree (a, Int), Store)
> mlabel (Leaf x) = do
>   s <- fresh
>   return $ Leaf (x, s)
